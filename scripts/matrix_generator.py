...
Comprehensive `ProductGenerator`-compatible fixture enumerating configuration combinations
across every `DataProduct` subclass (`Point2D`, `Scatter2D`, `Trace2D`, `AxLine`,
`HistogramEntry`, `TableEntry`) plus tags where several product types are deliberately mixed
...
pipeline gets exercised against every styling axis and every same-tag product-type
combination it needs to support.

`build_configuration_matrix` is the single entry point. Its signature matches
`ProductGenerator` (`Callable[[Path], ProductList]`), so it can be passed directly to
`generate_products`/`TrendifyPipeline.generate` like any real generator, or called directly
in a test and handed to `ProductStore.write_run`.
"""
...
from trendify.base.data_product import ProductList
...
def build_configuration_matrix(workdir: Path) -> ProductList:
    """
    Returns one comprehensive `ProductList` covering the styling/config matrix described in
    the module docstring.

    Args:
        workdir (Path): unused for data generation (this fixture doesn't read from disk);
            only `workdir.name` is used, to label metadata the way a real `ProductGenerator`
    ...
    _add_axis_scale_combinations(products)
    _add_grid_theme_combinations(products)
    _add_legend_location_combinations(products)
    _add_legend_option_combinations(products)
    _add_format2d_limit_combinations(products)
    _add_format2d_size_and_label_combinations(products)
    _add_pen_color_combinations(products)
    _add_pen_linestyle_combinations(products)
    _add_pen_field_combinations(products)
    _add_trace_marker_combinations(products)
    _add_marker_symbol_combinations(products)
    _add_marker_color_and_field_combinations(products)
    _add_scatter_configuration_combinations(products)
    _add_point_coordinate_type_combinations(products)
    _add_axline_orientation_combinations(products)
    _add_histogram_histtype_combinations(products)
    _add_histogram_bins_combinations(products)
    _add_histogram_alpha_combinations(products)
    _add_histogram_style_edge_case_combinations(products)
    _add_table_entry_value_type_combinations(products)
    _add_tag_shape_combinations(products)
    _add_metadata_combinations(products, run_label)
    _add_mixed_product_type_combinations(products)

    return products


def _add_axis_scale_combinations(products: ProductList) -> None:
    for scale_x, scale_y in [
        ...
        Trace2D.from_xy(
            tags=[("axis_scale_combinations", tag_name)],
            x=x,
            y=y,
            pen=Pen(label=tag_name),
            format2d=Format2D(scale_x=scale_x, scale_y=scale_y),
        ).append_to_list(products)


def _add_grid_theme_combinations(products: ProductList) -> None:
    grid_variants = {
        "grid_omitted": None,
    ...
    for tag_name, grid in grid_variants.items():
        Trace2D.from_xy(
            tags=[("grid_theme_combinations", tag_name)],
            x=_TIME,
            y=_rising_wave(),
            pen=Pen(label=tag_name),
            format2d=Format2D(grid=grid),
        ).append_to_list(products)


def _add_legend_location_combinations(products: ProductList) -> None:
    for loc in LegendLocation:
        tag_name = f"legend_loc_{loc.name.lower()}"
        Trace2D.from_xy(
            tags=[("legend_location_combinations", tag_name)],
            x=_TIME,
            y=_rising_wave(),
            pen=Pen(label=tag_name),
            format2d=Format2D(legend=Legend(loc=loc)),
        ).append_to_list(products)


def _add_legend_option_combinations(products: ProductList) -> None:
    legend_variants = {
    ...
    for tag_name, legend in legend_variants.items():
        Trace2D.from_xy(
            tags=[("legend_option_combinations", tag_name)],
            x=_TIME,
            y=_rising_wave(),
            pen=Pen(label=tag_name),
            format2d=Format2D(legend=legend),
        ).append_to_list(products)


def _add_format2d_limit_combinations(products: ProductList) -> None:
    limit_variants = {
        "lim_x_min_set": Format2D(lim_x_min=0.3),
        "lim_x_max_set": Format2D(lim_x_max=0.7),
        "lim_y_min_set": Format2D(lim_y_min=-0.5),
        "lim_y_max_set": Format2D(lim_y_max=0.5),
        "lim_x_and_y_all_set": Format2D(
            lim_x_min=0.2, lim_x_max=0.8, lim_y_min=-0.8, lim_y_max=0.8
        ),
    }
    for tag_name, format2d in limit_variants.items():
        Trace2D.from_xy(
            tags=[("axis_limit_combinations", tag_name)],
            x=_TIME,
            y=_rising_wave(),
            pen=Pen(label=tag_name),
            format2d=format2d,
...
def _add_format2d_size_and_label_combinations(products: ProductList) -> None:
    field_variants = {
        "figure_width_custom": Format2D(figure_width=12.0),
        "figure_height_custom": Format2D(figure_height=10.0),
        "title_fig_set": Format2D(title_fig="Figure title"),
        "title_ax_set": Format2D(title_ax="Axes title"),
        "label_x_set": Format2D(label_x="X axis label"),
        "label_y_set": Format2D(label_y="Y axis label"),
    }
    for tag_name, format2d in field_variants.items():
        Trace2D.from_xy(
            tags=[("format2d_field_combinations", tag_name)],
            x=_TIME,
            y=_rising_wave(),
            pen=Pen(label=tag_name),
            format2d=format2d,
...
def _add_pen_color_combinations(products: ProductList) -> None:
    color_variants = {
...
def _add_pen_linestyle_combinations(products: ProductList) -> None:
    linestyle_variants = {
        "pen_linestyle_solid": "-",
...
def _add_pen_field_combinations(products: ProductList) -> None:
    Trace2D.from_xy(
...
def _add_trace_marker_combinations(products: ProductList) -> None:
    Trace2D.from_xy(
...
def _add_marker_symbol_combinations(products: ProductList) -> None:
...
def _add_marker_color_and_field_combinations(products: ProductList) -> None:
    color_variants = {
...
def _add_scatter_configuration_combinations(products: ProductList) -> None:
...
def _add_point_coordinate_type_combinations(products: ProductList) -> None:
    Point2D(
...
def _add_axline_orientation_combinations(products: ProductList) -> None:
    AxLine(
...
def _add_histogram_histtype_combinations(products: ProductList) -> None:
...
def _add_histogram_bins_combinations(products: ProductList) -> None:
...
def _add_histogram_alpha_combinations(products: ProductList) -> None:
...
def _add_histogram_style_edge_case_combinations(products: ProductList) -> None:
...
def _add_table_entry_value_type_combinations(products: ProductList) -> None:
    TableEntry(
...
def _add_tag_shape_combinations(products: ProductList) -> None:
    Point2D(tags=["tag_shape_scalar_string"], x=1.0, y=1.0).append_to_list(products)
    Point2D(tags=[2024], x=1.0, y=1.0).append_to_list(products)
    Point2D(
        tags=[("tag_shape_tuple_of_strings", "nested_leaf")], x=1.0, y=1.0
    ).append_to_list(products)
    Point2D(
        tags=[("tag_shape_tuple_mixed_str_and_int", 2024)], x=1.0, y=1.0
    ).append_to_list(products)
    Point2D(
...
def _add_metadata_combinations(products: ProductList, run_label: str) -> None:
    Point2D(
        tags=[("metadata_combinations", "metadata_set")],
        x=1.0,
        y=1.0,
    ).append_to_list(products).set_metadata(
        {"run": run_label, "source": "product_matrix"}
    )
    Point2D(
...
def _add_mixed_product_type_combinations(products: ProductList) -> None:
    rng = np.random.default_rng(seed=5)

    trace_and_axline_tag = (
        "mixed_product_type_combinations",
    ...
    Point2D(tags=[point_and_histogram_tag], x=0.0, y=0.0).append_to_list(products)
    for value in rng.normal(size=30):
        HistogramEntry(
    ...
    format2d_present_and_omitted_tag = (
        "mixed_product_type_combinations",
        "format2d_present_and_omitted_shared_tag",
    )
    Point2D(tags=[format2d_present_and_omitted_tag], x=0.0, y=0.0).append_to_list(
        products
    )
    Trace2D.from_xy(
        tags=[format2d_present_and_omitted_tag],
        x=_TIME,
        y=_rising_wave(),
        pen=Pen(label="format2d_present_and_omitted_shared_tag"),
        format2d=Format2D(grid=Grid.from_theme(GridTheme.MATLAB)),
    ).append_to_list(products)
    ...
    Point2D(tags=[table_and_point_tag], x=1.0, y=1.0).append_to_list(products)
    ...
    Point2D(tags=[table_point_and_histogram_tag], x=1.0, y=1.0).append_to_list(products)
    for value in rng.normal(size=30):
        HistogramEntry(
            tags=[table_point_and_histogram_tag], value=float(value)
        ).append_to_list(products)

    every_product_type_tag = (
        "mixed_product_type_combinations",
        "table_point_scatter_trace_axline_and_histogram_shared_tag",
    )
    TableEntry(
        tags=[every_product_type_tag], row="row_1", col="col_1", value=1.0
    ).append_to_list(products)
    Point2D(
        tags=[every_product_type_tag],
        x=0.2,
        y=0.2,
        marker=Marker(label="sample_point"),
    ).append_to_list(products)
    ...
    for value in rng.normal(size=30):
        HistogramEntry(
            tags=[every_product_type_tag], value=float(value)
        ).append_to_list(products)
