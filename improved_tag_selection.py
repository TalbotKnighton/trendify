...
def _create_app_layout_with_dropdown(
    self, title: str, tab_contents: List[Dict]
) -> html.Div:
    ...
    dropdown_options = [
        {"label": tab["label"], "value": tab["value"]} for tab in tab_contents
    ]
    ...
    for tab in tab_contents:
        content_divs.append(
            html.Div(
                tab["content"], id=f"content-{tab['value']}", style={"display": "none"}
            )
        )
