...
import time
from typing import Union, List, Iterable, Any, Callable, Tuple, Type, Optional, TypeVar, Hashable, TYPE_CHECKING
...
class AssetSpec(HashableBase):
    ...
    def add_to_spec_registry(
        ...
        AssetSpecs.add_spec(spec=self, namespace=namespace, conflict_resolver=conflict_resolver)
        return self
    ...
    def get_spec(
        ...
        match get_spec_type_name(spec_type):
            case FigSpec.__name__:
                return AssetSpecs.get_spec(tag=self.tag_fig_spec, spec_type=FigSpec, namespace=self.namespace)
            case GridSpec.__name__:
                return AssetSpecs.get_spec(tag=self.tag_grid_spec, spec_type=GridSpec, namespace=self.namespace)
            case AxSpec.__name__:
                return AssetSpecs.get_spec(tag=self.tag_ax_spec, spec_type=AxSpec, namespace=self.namespace)
            case LegendSpec.__name__:
                return AssetSpecs.get_spec(tag=self.tag_legend_spec, spec_type=LegendSpec, namespace=self.namespace)
            case _:
...
class _AssetSpecs(BaseModel):
    ...
    @classmethod
    def _get_spec_tag_tuple(cls, tag: Tag, spec_type: Type[AssetSpec]|str, namespace: str = DEFAULT_NAMESPACE):
    ...
    def add_spec(
        ...
        existing_spec = self.get_spec(tag=spec.tag, spec_type=type(spec), namespace=namespace)
        ...
        if existing_spec is not None:
            match conflict_resolver:
                ...
                case _:
                    raise ValueError(f'Please specify a valid {AssetSpecConflictBehavior}')
    ...
    def get_specs(
        ...
        return {
            ...
            for k,v 
            in self.specs.items() 
            if self._filter_function(k=k, tag=tag, spec_type=spec_type, namespace=namespace)
        }
    ...
    def apply_to(
            ...
            output_type: OutputType = OutputType.matplotlib,
        ):
        match output_type:
            case OutputType.matplotlib:
                fig_specs = [fs for key, fs in AssetSpecs.get_specs(spec_type=FigSpec).items()]
                for fs in fig_specs:
                    fig = plt.figure(
                        figsize=fs.size,
                        dpi=fs.dpi,
                    )
                    linkers = [linker for key, linker in AssetSpecs.get_specs(spec_type=SpecLinker).items() if linker.tag_fig_spec == fs.tag]
                ...
                plt.show()
            case OutputType.plotly:
                pass
            case _:
                raise NotImplementedError("Have not yet implemented outputs for given type")
...
def _test():
    ...
    for t in collection.get_tags():
        AssetSpecs.add_single_axis_figure(
            t,
            product_tags=[t],
            title_fig=t
        )
    
    AssetSpecs.apply_to(collection=collection, output_type=OutputType.matplotlib)
