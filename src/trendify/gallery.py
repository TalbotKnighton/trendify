...
def generate_sine_wave_example():
    ...
    trace = Trace2D.from_xy(
        ...
        x=x,
        y=y,
        pen=Pen(color="blue", label="Noisy Sine", size=2),
        format2d=Format2D(
        ...
        if y_val > y[i] and y_val > y[i + 2]:  # Simple peak detection
            peaks.append(
                Point2D(
                    ...
                    x=x_val,
                    y=y_val,
                    marker=Marker(color="green", symbol="o", size=100, label="Peak"),
                )
            )
...
def generate_scatter_plot_example():
    ...
    cluster1 = [
        Point2D(
            ...
            x=x,
            y=y,
            marker=Marker(color="blue", symbol="o", size=50, label="Cluster 1"),
            format2d=format2d,
        )
    ...
    cluster2 = [
        Point2D(
            ...
            x=x,
            y=y,
            marker=Marker(color="red", symbol="o", size=50, label="Cluster 2"),
            format2d=format2d,
        )
    ...
    centroids = [
        Point2D(
            ...
            x=np.mean(x1),
            y=np.mean(y1),
            marker=Marker(color="darkblue", symbol="*", size=200, label="Centroid 1"),
            format2d=format2d,
        ),
        Point2D(
            tags=("clusters", "analysis"),
            x=np.mean(x2),
            y=np.mean(y2),
            marker=Marker(color="darkred", symbol="*", size=200, label="Centroid 2"),
            format2d=format2d,
        ),
    ]
...
def generate_histogram_example():
    ...
    normal_entries = [
        HistogramEntry(
            ...
            value=v,
            style=HistogramStyle(color="blue", label="Normal", alpha_face=0.3, bins=30),
            format2d=Format2D(
    ...
    uniform_entries = [
        HistogramEntry(
            tags=("distributions", "comparison"),
            value=v,
            style=HistogramStyle(color="red", label="Uniform", alpha_face=0.3, bins=30),
        )
    ...
    exp_entries = [
        HistogramEntry(
            ...
            style=HistogramStyle(
                color="green", label="Exponential", alpha_face=0.3, bins=30
            ),
        )
        for v in exp_data
    ]
