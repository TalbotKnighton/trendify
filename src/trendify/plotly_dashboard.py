...
class PlotlyDashboardGenerator:
    ...
    def _create_table_section(self, tag_str: str) -> html.Div:
        ...
        return html.Div([
            ...
            html.Div([
                dcc.Tabs([
                    dcc.Tab(label='Melted Table', children=[
                        dash_table.DataTable(
                            id=f'melted-table-{tag_str}',
                            columns=[{'name': col, 'id': col} for col in melted_df.columns],
                    ...
                    dcc.Tab(label='Pivot Table', disabled=pivot_df is None, children=[
                        dash_table.DataTable(
                            ...
                            columns=[
                                ...
                                for col in (pivot_df.columns if pivot_df is not None else [])
                            ],
                            data=pivot_df.reset_index().to_dict('records') if pivot_df is not None else [],
                            filter_action="native",
                            sort_action="native",
                            sort_mode="multi",
                            page_size=15,
                        ...
                        ) if pivot_df is not None else html.Div("Unable to create pivot table")
                    ]),
                ]),
            ])
        ])
            ...
            html.Div([
                ...
                html.Div([
                    dcc.Graph(
                        ...
                        style={'height': '600px'},
                        figure=self._create_histogram_figure(tag_str, histogram_groups, 20, ['show_kde', 'show_legend'])
                    )
                ], style={'width': '80%', 'display': 'inline-block'})
            ])
        ])
    
    def _create_xy_figure(self, tag_str: str, format2d: Optional[Format2D] = None) -> go.Figure:
        ...
        for point in data['points']:
            label = point.marker.label if point.marker and point.marker.label else "Points"
        ...
        layout_args = dict(
            title=format2d.title_ax if format2d and format2d.title_ax else str(data['tag']),
            xaxis=dict(
            ...
            plot_bgcolor='white',
            legend_title=format2d.title_legend if format2d and format2d.title_legend else None,
            showlegend=True,
        ...
        if format2d:
            if format2d.lim_x_min is not None:
                layout_args['xaxis']['range'] = [format2d.lim_x_min, layout_args['xaxis'].get('range', [None, None])[1]]
            if format2d.lim_x_max is not None:
                layout_args['xaxis']['range'] = [layout_args['xaxis'].get('range', [None, None])[0], format2d.lim_x_max]
            if format2d.lim_y_min is not None:
                layout_args['yaxis']['range'] = [format2d.lim_y_min, layout_args['yaxis'].get('range', [None, None])[1]]
            if format2d.lim_y_max is not None:
                layout_args['yaxis']['range'] = [layout_args['yaxis'].get('range', [None, None])[0], format2d.lim_y_max]
    ...
    def _create_histogram_figure(
        ...
        # Update layout
        layout_args = dict(
            title=format2d.title_ax if format2d and format2d.title_ax else f"Histogram - {tag_str}",
            barmode='overlay',
            xaxis=dict(
    ...
    def _register_callbacks(self):
        ...
        for tag_str in self.tag_data.keys():
            ...
            if self.tag_data[tag_str]['traces'] or self.tag_data[tag_str]['points'] or self.tag_data[tag_str]['axlines']:
                @self.app.callback(
            ...
            if self.tag_data[tag_str]['histograms']:
                ...
                def update_histogram(bins, options, tag_str=tag_str):
                    ...
                    fig = self._create_histogram_figure(tag_str, histogram_groups, bins, options)
                    return fig
