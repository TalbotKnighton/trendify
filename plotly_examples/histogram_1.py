...
def calculate_histogram_statistics(
    series: pd.Series, num_bins: int = 20
) -> dict[str, Any]:
    ...
    try:
        ...
        stats_dict = {
            ...
            "bins": num_bins,
            "bin_width": f"{(clean_series.max() - clean_series.min()) / num_bins:.3f}"
    ...
    [
        ...
        html.Div(
            [
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                html.Label(
                                    "Lock all bins to global setting:",
                                    style={
                ...
                html.Div(
                    [
                        dcc.Graph(id="histogram-plot", style={"height": "700px"}),
                    ],
                    style={
                        "width": "50%",
                        "display": "inline-block",
        ...
        html.Div(
            [
                ...
                html.Button(
                    "Close", id="close-color-picker", style={"marginTop": "10px"}
                ),
            ],
        ...
        dcc.Store(
            id="global-bins-active", data=True
        ),  # True by default# Add this to your layout
        dcc.Interval(
...
@app.callback(
    Output("global-bins-active", "data"), [Input("global-bins-toggle", "value")]
)
...
@app.callback(
    [Output("color-picker-panel", "style"), Output("active-series", "data")],
    [
...
def toggle_color_picker(button_clicks, close_clicks, button_ids, current_style):
    ...
    if "color-button" in triggered_id and button_clicks and any(button_clicks):
        try:
            ...
            button_idx = next(
                (i for i, clicks in enumerate(button_clicks) if clicks), None
            )
...
def update_series_color(color_clicks, color_ids, active_series, current_colors):
    ...
    triggered_id = ctx.triggered[0]["prop_id"]
    if "color-option" not in triggered_id or not any(
        click for click in color_clicks if click
    ):
        return dash.no_update
            ...
            try:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        marker=dict(
                            symbol="line-ns", size=10, color=color, opacity=0.5
                        ),
                        showlegend=False,
                        hoverinfo="x",
                    )
...
app.clientside_callback(
    ...
    [Input("custom-legend", "children"), Input("series-data", "data")],
)
...
@app.callback(
    [Output("series-colors", "data"), Output("series-data", "data")],
    [Input("histogram-columns", "value"), Input("bins-settings", "data")],
    [State("series-colors", "data")],
)
def initialize_and_update_series_data(selected_cols, bins_settings, existing_colors):
    ...
    ctx = dash.callback_context
    debug_log(
        f"Series data update triggered by: {ctx.triggered[0]['prop_id'] if ctx.triggered else 'none'}"
    )
...
def update_all_bin_settings(
    bin_values, slider_value, selected_cols, bin_ids, current_settings, global_active
):
